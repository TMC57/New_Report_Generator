...
odoo_products = models.execute_kw(
    ...
    [[]],
    {'fields': ["id", "price", "x_studio_id_webflow", "name", "company_id"], 'limit': 1000000}
)

# Filtrage par company_id == [1, 'TMH Corporation SA']
odoo_products = [
    prod for prod in odoo_products
    if prod.get("company_id") == [1, "TMH Corporation SA"]
]

print(f"✅ Produits Odoo récupérés pour TMH Corporation SA : {len(odoo_products)}")
...
print(f"\n 🎯 Nombre de prix mis à jour : {PriceUpdateNbr} \n")
