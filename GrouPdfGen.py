...
def build_owner_totals_table(owner_block: dict) -> Table:
    ...
    # Largeurs dynamiques
    TOTAL_W = 25 * cm
    MIN_PROD_W, MAX_PROD_W = 6.0 * cm, 11.0 * cm
...
def build_group_consumption_table(owner_block: dict) -> Table:
    ...
    facilities = owner_block.get("facilities", []) or []
    facility_labels_raw = [_short_facility_name(f.get("facilityName", "")) for f in facilities]
    ...
    header_row = [Paragraph("Produits", head_style)] + [Paragraph(lbl, head_style) for lbl in facility_labels_raw]
    data = [header_row]
...
def generate_group_stock_chart(owner_stock_block: dict) -> BytesIO | None:
    ...
    plt.subplots_adjust(left=0.06, right=0.99, top=0.92, bottom=bottom)
...
def build_group_stock_table(owner_stock_block: dict) -> Table:
    ...
    data = [[Paragraph("Produits", head_style)] + [Paragraph(lbl, head_style) for lbl in facility_labels]]
